...
bin/ai-triage-pod --context-file notes.txt <namespace> <pod-name>
printf 'extra context' | bin/ai-triage-pod <namespace> <pod-name>
```

Optional inputs are appended to the generated triage prompt:

- `--context-file FILE` appends the contents of `FILE`
- stdin is appended when the command receives piped input
