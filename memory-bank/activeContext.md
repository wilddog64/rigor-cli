...
**Status:** v0.1.5 shipped; enforce_admins restored; pyjenkinsapi branch protection set up; v0.1.6 now has the `ai-triage-pod` helper added.

## Recent Changes
- **`ai-triage-pod` done** — commit `4f47a2c` adds the generic Kubernetes pod triage wrapper for `rigor-cli` to mirror the k3d-manager Copilot helper while using the repo’s `ai-review` backend and accepting stdin / file context. Docs updated in `docs/howto/ai-triage-pod.md`, `docs/howto/use-ai-helpers.md`, and the ai-helpers gist. Tests and shellcheck passed.
